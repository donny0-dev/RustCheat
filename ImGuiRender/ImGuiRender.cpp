...
void GRenderer::Initialize()
{
	...
	if (!m_pNormal)
		m_pNormal = io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\verdana.ttf", 22.f, &FontConfig, io.Fonts->GetGlyphRangesCyrillic());
	if (!m_pBold)
	...
	//{
		...
		//m_pIconFont = io.Fonts->AddFontFromFileTTF(FONT_ICON_FILE_NAME_FK, 16.0f, &icons_config, icons_ranges);
	//}
}
...
float GRenderer::DrawTextGui(const std::string& text, const ImVec2& pos, float size, const ImColor& color, bool center, ImFont* fn)
{
	...
	ImFont* font = m_pNormal;

	if (fn)
		font = fn;

	ImVec2 textSize = m_pNormal->CalcTextSizeA(size, FLT_MAX, 0.0f, text.c_str());
	if (center)
	{

		draw->AddText(font, size, ImVec2(pos.x - textSize.x / 2.0f, pos.y + textSize.y), ImGui::GetColorU32(ImVec4(color.Value.x, color.Value.y, color.Value.z, color.Value.w)), text.c_str());

	}
	else
	{

		draw->AddText(font, size, ImVec2(pos.x, pos.y + textSize.y), ImGui::GetColorU32(ImVec4(color.Value.x, color.Value.y, color.Value.z, color.Value.w)), text.c_str());
